...
QUnit.test('Testing function with several sets of inputs', function (assert) {
  assert.equal(calculateResult(5, 5), 25, 'When ever we multiple 5 and 5 we get  results as 25');
  assert.throws(function () { calculateResult(null); }, /The given argument is not a number/, 'Passing in null correctly raises an Error');
  assert.equal(calculateResult(-5, -5), 25, 'when ever we multiply -5 and -5 we get 25');
  assert.throws(function () { calculateResult(null); }, /The given argument is not a number/, 'Passing in null correctly raises an Error');
  assert.throws(function () { takeValue('G'); }, /The given value is not a number/, 'Passing other than number');
